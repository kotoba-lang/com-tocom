(ns tocom.main
  "Kotodama WASM entrypoint for the Tocom clean-room actor (L4) — Clojure port.
  ...
  facts (`tocom.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"tocom.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "tocom-compat" :tier tier :entities entities} 200])
